...
class ConvFeatureExtractor(nn.Module):
    def __init__(self, in_channels: int, base_filters: int, kernel_size: int,
                 conv_blocks: int, embed_dim: int) -> None:
        super().__init__()
...
def compute_stft_features(x: torch.Tensor, cfg: STFTConfig) -> torch.Tensor:
    ...
    eeg = x[:, :, 0, :]  # use the first channel for spectral features
...
class CausalTransformerClassifier(nn.Module):
    def __init__(self, cfg: EEGTransformerConfig) -> None:
        ...
        self.pos_embed = nn.Parameter(
            torch.randn(1, self.context_window, cfg.d_model) / math.sqrt(cfg.d_model))
