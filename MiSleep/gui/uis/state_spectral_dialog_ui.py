# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file '.\state_spectral_dialog.ui'
#
# Created by: PyQt5 UI code generator 5.15.10
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_StateSpectralDialog(object):
    def setupUi(self, StateSpectralDialog):
        StateSpectralDialog.setObjectName("StateSpectralDialog")
        StateSpectralDialog.resize(400, 600)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/logo/logo.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        StateSpectralDialog.setWindowIcon(icon)
        self.gridLayout_2 = QtWidgets.QGridLayout(StateSpectralDialog)
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.OKBt = QtWidgets.QPushButton(StateSpectralDialog)
        self.OKBt.setObjectName("OKBt")
        self.gridLayout_2.addWidget(self.OKBt, 2, 0, 1, 1)
        self.CancelBt = QtWidgets.QPushButton(StateSpectralDialog)
        self.CancelBt.setObjectName("CancelBt")
        self.gridLayout_2.addWidget(self.CancelBt, 2, 1, 1, 1)
        self.groupBox_2 = QtWidgets.QGroupBox(StateSpectralDialog)
        self.groupBox_2.setObjectName("groupBox_2")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.groupBox_2)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.StartTimeCheckBox = QtWidgets.QCheckBox(self.groupBox_2)
        self.StartTimeCheckBox.setObjectName("StartTimeCheckBox")
        self.gridLayout_3.addWidget(self.StartTimeCheckBox, 0, 0, 1, 1)
        self.StartTimeEditor = QtWidgets.QDateTimeEdit(self.groupBox_2)
        self.StartTimeEditor.setObjectName("StartTimeEditor")
        self.gridLayout_3.addWidget(self.StartTimeEditor, 1, 0, 1, 1)
        self.EndTimeCheckBox = QtWidgets.QCheckBox(self.groupBox_2)
        self.EndTimeCheckBox.setObjectName("EndTimeCheckBox")
        self.gridLayout_3.addWidget(self.EndTimeCheckBox, 2, 0, 1, 1)
        self.HourSegmentCheckBox = QtWidgets.QCheckBox(self.groupBox_2)
        self.HourSegmentCheckBox.setObjectName("HourSegmentCheckBox")
        self.gridLayout_3.addWidget(self.HourSegmentCheckBox, 4, 0, 1, 1)
        self.EndTimeEditor = QtWidgets.QDateTimeEdit(self.groupBox_2)
        self.EndTimeEditor.setObjectName("EndTimeEditor")
        self.gridLayout_3.addWidget(self.EndTimeEditor, 3, 0, 1, 1)
        self.gridLayout_2.addWidget(self.groupBox_2, 0, 0, 1, 2)
        self.groupBox = QtWidgets.QGroupBox(StateSpectralDialog)
        self.groupBox.setObjectName("groupBox")
        self.gridLayout = QtWidgets.QGridLayout(self.groupBox)
        self.gridLayout.setSpacing(10)
        self.gridLayout.setObjectName("gridLayout")
        self.BPLow = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.BPLow.setDecimals(1)
        self.BPLow.setMaximum(100000.0)
        self.BPLow.setSingleStep(0.1)
        self.BPLow.setProperty("value", 0.5)
        self.BPLow.setObjectName("BPLow")
        self.gridLayout.addWidget(self.BPLow, 4, 1, 1, 1)
        self.BPHigh = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.BPHigh.setDecimals(1)
        self.BPHigh.setMaximum(10000.0)
        self.BPHigh.setSingleStep(0.1)
        self.BPHigh.setProperty("value", 30.0)
        self.BPHigh.setObjectName("BPHigh")
        self.gridLayout.addWidget(self.BPHigh, 4, 2, 1, 1)
        self.ArtThresholdSpinBox = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.ArtThresholdSpinBox.setDecimals(1)
        self.ArtThresholdSpinBox.setProperty("value", 2.0)
        self.ArtThresholdSpinBox.setObjectName("ArtThresholdSpinBox")
        self.gridLayout.addWidget(self.ArtThresholdSpinBox, 13, 2, 1, 1)
        self.GaussianCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.GaussianCheckBox.setObjectName("GaussianCheckBox")
        self.gridLayout.addWidget(self.GaussianCheckBox, 8, 0, 1, 2)
        self.MergeDataCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.MergeDataCheckBox.setObjectName("MergeDataCheckBox")
        self.gridLayout.addWidget(self.MergeDataCheckBox, 14, 0, 1, 2)
        self.WinLengthCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.WinLengthCheckBox.setObjectName("WinLengthCheckBox")
        self.gridLayout.addWidget(self.WinLengthCheckBox, 5, 0, 1, 2)
        self.ChannelSelector = QtWidgets.QComboBox(self.groupBox)
        self.ChannelSelector.setObjectName("ChannelSelector")
        self.gridLayout.addWidget(self.ChannelSelector, 1, 0, 1, 3)
        self.RelativeCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.RelativeCheckBox.setObjectName("RelativeCheckBox")
        self.gridLayout.addWidget(self.RelativeCheckBox, 12, 0, 1, 2)
        self.WinLengthSpinBox = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.WinLengthSpinBox.setMinimum(0.1)
        self.WinLengthSpinBox.setMaximum(100.0)
        self.WinLengthSpinBox.setSingleStep(0.1)
        self.WinLengthSpinBox.setProperty("value", 10.0)
        self.WinLengthSpinBox.setObjectName("WinLengthSpinBox")
        self.gridLayout.addWidget(self.WinLengthSpinBox, 5, 2, 1, 1)
        self.BPFilterCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.BPFilterCheckBox.setObjectName("BPFilterCheckBox")
        self.gridLayout.addWidget(self.BPFilterCheckBox, 4, 0, 1, 1)
        self.label = QtWidgets.QLabel(self.groupBox)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 0, 0, 1, 1)
        self.RejectArtifactCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.RejectArtifactCheckBox.setObjectName("RejectArtifactCheckBox")
        self.gridLayout.addWidget(self.RejectArtifactCheckBox, 13, 0, 1, 2)
        self.GaussianSpinBox = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.GaussianSpinBox.setMinimum(0.1)
        self.GaussianSpinBox.setMaximum(20.0)
        self.GaussianSpinBox.setProperty("value", 2.0)
        self.GaussianSpinBox.setObjectName("GaussianSpinBox")
        self.gridLayout.addWidget(self.GaussianSpinBox, 8, 2, 1, 1)
        self.nfftCheckBox = QtWidgets.QCheckBox(self.groupBox)
        self.nfftCheckBox.setObjectName("nfftCheckBox")
        self.gridLayout.addWidget(self.nfftCheckBox, 6, 0, 1, 2)
        self.nfftSpinBox = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.nfftSpinBox.setMinimum(0.2)
        self.nfftSpinBox.setMaximum(100.0)
        self.nfftSpinBox.setProperty("value", 10.0)
        self.nfftSpinBox.setObjectName("nfftSpinBox")
        self.gridLayout.addWidget(self.nfftSpinBox, 6, 2, 1, 1)
        self.gridLayout_2.addWidget(self.groupBox, 1, 0, 1, 2)

        self.retranslateUi(StateSpectralDialog)
        QtCore.QMetaObject.connectSlotsByName(StateSpectralDialog)

    def retranslateUi(self, StateSpectralDialog):
        _translate = QtCore.QCoreApplication.translate
        StateSpectralDialog.setWindowTitle(_translate("StateSpectralDialog", "Dialog"))
        self.OKBt.setText(_translate("StateSpectralDialog", "OK"))
        self.CancelBt.setText(_translate("StateSpectralDialog", "Cancel"))
        self.groupBox_2.setTitle(_translate("StateSpectralDialog", "Time options"))
        self.StartTimeCheckBox.setText(_translate("StateSpectralDialog", "Start time"))
        self.StartTimeEditor.setDisplayFormat(_translate("StateSpectralDialog", "yyyy/MM/dd HH:mm:ss"))
        self.EndTimeCheckBox.setText(_translate("StateSpectralDialog", "Stop time"))
        self.HourSegmentCheckBox.setText(_translate("StateSpectralDialog", "Hour segmentation"))
        self.EndTimeEditor.setDisplayFormat(_translate("StateSpectralDialog", "yyyy/MM/dd HH:mm:ss"))
        self.groupBox.setTitle(_translate("StateSpectralDialog", "State spectral analysis options"))
        self.GaussianCheckBox.setText(_translate("StateSpectralDialog", "Gaussian smmothing(sigma)"))
        self.MergeDataCheckBox.setText(_translate("StateSpectralDialog", "Merge data"))
        self.WinLengthCheckBox.setText(_translate("StateSpectralDialog", "Window length(s)"))
        self.RelativeCheckBox.setText(_translate("StateSpectralDialog", "Relative"))
        self.BPFilterCheckBox.setText(_translate("StateSpectralDialog", "Bandpass filter"))
        self.label.setText(_translate("StateSpectralDialog", "Channel:"))
        self.RejectArtifactCheckBox.setText(_translate("StateSpectralDialog", "Reject artifact"))
        self.nfftCheckBox.setText(_translate("StateSpectralDialog", "nfft(s, > window length)"))
from misleep.gui.resources import misleep
